...
class Game:
    ...
    def calc_possible_moves(self, player):
        ...
        for coor in self.pieces_of(player):
            ...
            for vector in DIRECTIONS:
                copy = cp.copy(piece)
                while (self.valid_coordinates(copy)
                    and self.get_piece(copy.x, copy.y) != None): # While there are pieces
                    # Proceed in that direction
                    copy.x = copy.x + vector[0]
                    copy.y = copy.y + vector[1]
                    if (self.valid_coordinates(copy)
                        and self.get_piece(copy.x, copy.y) == None # There is a blank
                        and self.get_piece(copy.x - vector[0], copy.y - vector[1]).player != player # Previous was an enemy piece
                        and (copy.x, copy.y) not in moves): # and not registered move yet
    ...
    def post_play(self, last_piece):
        ...
        for vector in DIRECTIONS:
            ...
            while(self.valid_coordinates(copy)
                and self.get_piece(copy.x, copy.y) != None):
                ...
                if (self.valid_coordinates(copy)
                    and self.get_piece(copy.x, copy.y) != None): # There is a piece
